...
//  Tinderable.swift
//
//  Created by Orkhan Alizade on 23/09/15.
//  Copyright (c) 2015 Orkhan Alizade. All rights reserved.
//

import UIKit

class TinderablePanGestureRecognizer: UIPanGestureRecognizer {
    
}

public func ==(lhs: TinderableViewDirection, rhs: TinderableViewDirection) -> Bool {
    return lhs.rawValue == rhs.rawValue
}

public struct TinderableViewDirection : OptionSetType, CustomStringConvertible {
    ...
    public static var allZeros: TinderableViewDirection {
        return self.init(rawValue: 0)
    }

    static var None: TinderableViewDirection       { return self.init(rawValue: 0b0000) }
    static var Left: TinderableViewDirection       { return self.init(rawValue: 0b0001) }
    static var Right: TinderableViewDirection      { return self.init(rawValue: 0b0010) }
    static var Up: TinderableViewDirection         { return self.init(rawValue: 0b0100) }
    static var Down: TinderableViewDirection       { return self.init(rawValue: 0b1000) }
    static var Horizontal: TinderableViewDirection { return [TinderableViewDirection.Left,  TinderableViewDirection.Right] }
    static var Vertical: TinderableViewDirection   { return [TinderableViewDirection.Up, TinderableViewDirection.Down] }
    static var All: TinderableViewDirection        { return [TinderableViewDirection.Horizontal, TinderableViewDirection.Vertical] }
    
    static func fromPoint(point: CGPoint) -> TinderableViewDirection {
    ...
    public var description: String {
        switch self {
        case TinderableViewDirection.None:
            return "None"
        case TinderableViewDirection.Left:
            return "Left"
        case TinderableViewDirection.Right:
            return "Right"
        case TinderableViewDirection.Up:
            return "Up"
        case TinderableViewDirection.Down:
            return "Down"
        case TinderableViewDirection.Horizontal:
            return "Horizontal"
        case TinderableViewDirection.Vertical:
            return "Vertical"
        case TinderableViewDirection.All:
            return "All"
        default:
            return "Unknown"
        }
...
public class TinderableView: UIView {
    // MARK: - Public
    // MARK: Data Source
    public var numPrefetchedViews = 3
    public var nextView: (() -> UIView?)?
    // MARK: Animation
    public var animateView: (view: UIView, index: Int, views: [UIView], swipeableView: TinderableView) -> () = {
        func toRadian(degree: CGFloat) -> CGFloat {
            return degree * CGFloat(M_PI/100)
        }
        func rotateView(view: UIView, forDegree degree: CGFloat, duration: NSTimeInterval, offsetFromCenter offset: CGPoint, swipeableView: TinderableView) {
        ...
        return {(view: UIView, index: Int, views: [UIView], swipeableView: TinderableView) in
    ...
    public var didSwipe: ((view: UIView, inDirection: TinderableViewDirection, directionVector: CGVector) -> ())?
    ...
    public var direction = TinderableViewDirection.Horizontal

    public var interpretDirection: (topView: UIView, direction: TinderableViewDirection, views: [UIView], swipeableView: TinderableView) -> (CGPoint, CGVector) = {(topView: UIView, direction: TinderableViewDirection, views: [UIView], swipeableView: TinderableView) in
    ...
    public func swipeTopView(inDirection direction: TinderableViewDirection) {
    ...
    public func swipeTopView(fromPoint location: CGPoint, inDirection directionVector: CGVector) {
        if let topView = topView() {
            let direction = TinderableViewDirection.fromPoint(CGPoint(x: directionVector.dx, y: directionVector.dy))
            swipeTopView(topView, direction: direction, location: location, directionVector: directionVector)
        }
    }
    private func swipeTopView(topView: UIView, direction: TinderableViewDirection, location: CGPoint, directionVector: CGVector) {
        unsnapView()
    ...
    public func insertTopView(view: UIView, fromPoint point: CGPoint) {
        if views.contains(view) {
            ...
            view.addGestureRecognizer(TinderablePanGestureRecognizer(target: self, action: Selector("handlePan:")))
    ...
    private func removeFromContainerView(aView: UIView) {
        for gestureRecognizer in aView.gestureRecognizers as [UIGestureRecognizer]! {
            if gestureRecognizer.isKindOfClass(TinderablePanGestureRecognizer.classForCoder()) {
    ...
    func handlePan(recognizer: UIPanGestureRecognizer) {
        ...
        case .Ended, .Cancelled:
            ...
            let directionChecked = [TinderableViewDirection.fromPoint(translation), direction] != TinderableViewDirection.None
...
//                pushView(topView, fromPoint: location, inDirection: directionVector)
//                removeFromViews(topView)
//                didSwipe?(view: topView, inDirection: TinderableViewDirection.fromPoint(translation))
//                loadViews()
            } else {
    ...
    private func pushView(aView: UIView, fromPoint point: CGPoint, inDirection direction: CGVector) {
        let anchorView = UIView(frame: CGRect(x: 0, y: 0, width: TinderableView.anchorViewWidth, height: TinderableView.anchorViewWidth))
        anchorView.center = point
